(function(_win, $) {
              ...
              $functSetTimer:function(_bool,_interval) {
                      ...
                      if (_bool) {
                            this.$timer  = setInterval(function() {
                              $.getJSON(
                                  'kyussam-tweets/tweets_json.php?count=10',function(data) {
                                      listTweets(data);
                                  });
                  ...
                  toggleShowTweet:function (_bool) {
                      ...
                      if (_bool) {
                          ...
                          setTimeout(function(){$('.tweet-wrapper,.twitter-link').addClass('sho')},2000);
                          return;
                      };
    ...
    function displayFoodImage(_box,_bool) {
        var $box = $(_box),
          ...
          if (_bool) {
              $scn.css({backgroundImage: $box.css('backgroundImage')}).addClass('blur').removeClass('act-desc');
          };
        ...
        $('#display_screen').find('.txt-desc').html(getFoodDesc($index));
          var $bg = $('.icon-pop-food' + $index).data('src');
              ...
              if (setFadeHeight(getDocHeight(),true)) {
                $scn.css({ backgroundImage: 'url(' + $bg + ')' }).removeClass('blur').addClass('act-desc');
              };
    };
    ...
    function checkHashUrl() {
        ...
        if ($url.indexOf('/#!/menu') !== -1 || $url.indexOf('/menu') !== -1) {
            setTimeout(function(){$('body').removeClass('ready').addClass('showFade').addClass('showMenu')},200);
            return;
        };
    };
      ...
      $('.icon-pop-food').each(function(i){
        ...
        $('.box-photo').on('click', function() {
            var $box = $(this);
                ...
                $('#display_screen').css({'backgroundImage': 'url(imgs/food/full/photo' + $box.index() + '.png)'})
                                    .find('.txt-desc')
